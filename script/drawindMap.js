...
$(function() {
 ...
 function update_select(state){
   document.getElementById('allstates').value=state.value;
   jQuery("#chart-india .highcharts-title tspan").text("GST Statistics (" + state.name+")");
 }
 
 jQuery("#chart-india").on('click', function(e){
  //console.log(e.target.tagName)
  // console.log(this)
   if(e.target.tagName=="path"){
     console.log("in the body")
   }else{
     console.log("out of the body")
   }
   
 })
 function addstop() {
  $('head').append('<style id="xr" type="text/css">.myclass{pointer-events: none;}</style>');
}
function removestop() {
  $('head').find('#xr').remove();
}
 function changeBackground(e){
   ...
   let pathx = document.querySelectorAll("#chart-india path");
   console.log(pathx);
  $(pathx).off("click",changeBackground);
  addstop();
  $("path").attr('class', 'myclass');
 
  setTimeout(function(){
    console.log("varun")
    removestop();
    // $("pathx").attr('class', 'test');
    //$(pathx).on("click",changeBackground);
}, 2000);
